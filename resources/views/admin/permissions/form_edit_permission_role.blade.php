...
<form name="edit_role_form" role="form" action="{{ route('admin.updatePermissionForRole', $roles->id) }}" class="form-horizontal" method="POST"
            ...
            <div class="col-md-12">
                ...
                <div class="form-group">
                    <label class="col-md-6" style="margin-top: 1%; color:#585247"
                                    for="Date Of Birth">Perfil</label>
                                   
                                    <input id="get_created_role" name="role_description" type="text" placeholder="Sobrenome"
                                    class="form-control {{ $errors->has('edit_last_name') ? ' is-invalid' : '' }}" value="{{ $roles->name }}">
                ...
                <div class="form-group">
                    <label class="col-md-6" style="margin-top: 1%; color:#585247"
                                ...
                                <select id="choices-multiple-remove-button" multiple name="perm[]" id="select"
                                    ...
                                    @foreach ($roles->permissions as $permOld)
                                    
                                    <option value="{{ $permOld->id }}" @selected($permOld->name == $permOld->name)>{{$permOld->name}}
                                    </option>
                                    ...
                                    <option value="{{ $perm->id }}" @disabled($perm->id == $roles->permissions->pluck('id')->contains($perm->id))>{{$perm->name}}</option>
